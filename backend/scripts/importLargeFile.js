...
async function importBatch(connection, profiles) {
  const profileValues = [];
  ...
  if (experienceValues.length > 0) {
    const expSql = `INSERT IGNORE INTO work_experience 
  ...
  if (educationValues.length > 0) {
    const eduSql = `INSERT IGNORE INTO education 
  ...
  if (certificationValues.length > 0) {
    const certSql = `INSERT IGNORE INTO certifications 
  ...
  if (companyConnectionValues.length > 0) {
    const companySql = `INSERT IGNORE INTO company_connections 
...
async function main() {
  ...
  if (!jsonFilePath) {
    console.error('Usage: node importLargeFile.js <path-to-json-file>');
    process.exit(1);
  }

  console.log('🚀 Starting LinkedIn data import (streaming)...');
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.on('data', async (profile) => {
      batch.push(profile);
      
      if (batch.length >= BATCH_SIZE) {
        stream.pause();
        const imported = await importBatch(connection, batch);
        totalProcessed += imported;
        console.log(`📦 Batch ${batchNum}: ${imported} profiles | Total: ${totalProcessed.toLocaleString()}`);
        batch = [];
        batchNum++;
        stream.resume();
      }
    ...
    stream.on('end', async () => {
      ...
      if (batch.length > 0) {
        const imported = await importBatch(connection, batch);
        totalProcessed += imported;
        console.log(`📦 Final batch: ${imported} profiles | Total: ${totalProcessed.toLocaleString()}`);
      }

      await connection.end();
      console.log(`\n✅ Import completed!`);
      console.log(`📊 Total profiles: ${totalProcessed.toLocaleString()}`);
      resolve();
    });
